...
interface RainbowButtonProps
  ...
  asChild?: boolean
  speed?: number | string
  hoverSpeed?: number | string
}

const RainbowButton = React.forwardRef<HTMLButtonElement, RainbowButtonProps>(
  ({ className, variant, size, asChild = false, speed, hoverSpeed, style, ...props }, ref) => {
    const Comp = asChild ? Slot : "button"
    const resolvedSpeed =
      speed === undefined ? undefined : typeof speed === "number" ? `${speed}s` : speed
    const resolvedHoverSpeed =
      hoverSpeed === undefined
        ? undefined
        : typeof hoverSpeed === "number"
          ? `${hoverSpeed}s`
          : hoverSpeed
    const mergedStyle =
      resolvedSpeed === undefined && resolvedHoverSpeed === undefined
        ? style
        : ({
            ...style,
            ...(resolvedSpeed !== undefined ? { "--speed": resolvedSpeed } : {}),
            ...(resolvedHoverSpeed !== undefined ? { "--hover-speed": resolvedHoverSpeed } : {}),
    ...
    return (
      <Comp
        ...
        className={cn(
          ...
          resolvedHoverSpeed !== undefined && "hover:[--speed:var(--hover-speed)]"
        )}
        style={mergedStyle}
        ref={ref}
        {...props}
